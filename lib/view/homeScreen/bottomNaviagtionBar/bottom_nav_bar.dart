...
class _BottomNavigationBarWidgetState
  ...
  void initState() {
    ...
    const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    ChannelId = List.generate(5, (i) => chars[Random().nextInt(chars.length)])
        .join();
  }

  Future<void> _loadBlockStatus() async {
    // 1) Fetch the user's own doc
    final userSnap = await FirebaseFirestore.instance
        .collection('UserEntity')
        .doc(FirebaseAuth.instance.currentUser!.uid)
        .get();
    final bool userBlocked = (userSnap.data()?['isBlocked'] as bool?) ?? false;

    // 2) Check if *any* livestream owned by this user is blocked
    final liveQuery = await FirebaseFirestore.instance
        .collection('livestreams')
        .where('adminFirebaseId', isEqualTo: FirebaseAuth.instance.currentUser!.uid)
        .where('isBlocked', isEqualTo: true)
        .limit(1)
        .get();
    final bool liveBlocked = liveQuery.docs.isNotEmpty;

    // 3) Check if *any* product created by this user is blocked
    final prodQuery = await FirebaseFirestore.instance
        ...
        .limit(1)
        .get();
    final bool prodBlocked = prodQuery.docs.isNotEmpty;

    // 4) Combine them
    if (!mounted) return;
    setState(() {
      isBlocked = userBlocked || liveBlocked || prodBlocked;
    });
  }
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              if (isBlocked)
                ...
                GestureDetector(
                  ...
                  child:
                  _buildOption(Icons.local_offer, "create_product".tr),
                ),
                GestureDetector(
                  onTap: () async {
                    Get.back();
                    ...
                    final adminName = data['firstName'] ?? 'Admin';
                    final adminPhoto = data['image'] ??
                        'https://images.unsplash.com/photo-1541516160071-4bb0c5af65ba?...';
                    final interests = (data['interests'] as List?) ?? [];
                    final category =
                    interests.isNotEmpty ? interests[0] : 'General';
                  ...
                  child:
                  _buildOption(Icons.play_circle_fill, "schedule_show".tr),
                ),
                GestureDetector(
                  onTap: () {
                    Get.back();
                  ...
                  child: _buildOption(Icons.store, "seller_hub".tr,
                      subtitle: "seller_hub_subtitle".tr),
                ),
              ],
            ]),
