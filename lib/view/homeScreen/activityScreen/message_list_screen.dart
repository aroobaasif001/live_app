...
class MessagesList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.white,
      body: StreamBuilder<QuerySnapshot>(
        // 1) Only get chats where I participate
        ...
        builder: (context, chatSnap) {
          ...
          // 2) Pull out the other user ID from each chat doc
          final otherUserIds = chatSnap.data!.docs
              .map((doc) {
          ...
          // 3) Fetch those users’ profiles in one go
          return FutureBuilder<QuerySnapshot>(
            ...
            builder: (context, userSnap) {
              ...
              if (!userSnap.hasData || userSnap.data!.docs.isEmpty) {
                return Center(child: Text("Users not found".tr));
              }

              // Build a list of RegistrationEntity from the results
              final users = userSnap.data!.docs.map((doc) {
                final d = doc.data()! as Map<String, dynamic>;
                return RegistrationEntity(
                  regId: doc.id,
                  firstName: d['firstName'] ?? 'Unknown',
                  image: d['image'] ?? applegImage,
                  subscribers:
                  List<String>.from(d['subscribers'] ?? <String>[]),
                );
              }).toList();
              ...
              return ListView.separated(
                ...
                separatorBuilder: (_, __) => Divider(color: Color(0xffe2e2e2)),
                itemBuilder: (context, i) => _buildUserTile(context, users[i]),
              );
            },
          );
        },
      ...
      floatingActionButton: Container(
        ...
        child: FloatingActionButton.extended(
          onPressed: () => showNewMessageBottomSheet(context),
          icon: Image.asset(penIcon),
          label: Text("Write".tr, style: TextStyle(color: Colors.white)),
          backgroundColor: Colors.transparent,
        ),
  ...
  Widget _buildUserTile(BuildContext context, RegistrationEntity user) {
    return ListTile(
      leading: ClipOval(
        child: Image.network(
          user.image ?? applegImage,
          ...
          errorBuilder: (_, __, ___) =>
              Image.asset(applegImage, height: 40, width: 40),
        ),
      ),
      title: CustomText(
        text: user.firstName ?? '',
      ...
      // trailing: CustomText(
      //   text: "4 d.", // you could replace this with the actual timestamp
      //   fontWeight: FontWeight.w500,
      //   fontFamily: 'Gilroy-Bold',
      //   fontSize: 16,
      //   color: Colors.grey,
